...
async function sendRequestAlwaysAuthenticated(
  ...
  const loginToken = (req && req.headers["x-cf-login-token"]) ? req.headers["x-cf-login-token"] : null;
  if (loginToken) {
...
async function sendRequestAuthenticated(
  ...
  const loginToken = req.headers["x-cf-login-token"];

  // get the tokens
  if (token) request["J_API_ACCESS_TOKEN"] = token;
  else if (refreshToken) request["J_API_REFRESH_TOKEN"] = refreshToken;
  else if (loginToken) request["J_API_LOGIN_TOKEN"] = loginToken;

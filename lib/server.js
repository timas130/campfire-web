 ...
 */
function sendRequest(name, request, media = false) {
  return new Promise((resolve, reject) => {
    ...
    socket.on("error", () => {
      reject("unknown remote server error. THEY ARE FUCKING DOWN!");
      socket.destroy();
    });
 ...
 * @returns {Promise<{J_STATUS: string, J_RESPONSE: {[p: string]: *}, [p: string]: *} | Buffer>} the server response
 */
async function sendRequestAlwaysAuthenticated(req, res, name, request = {}) {
 ...
 * @returns {Promise<{J_STATUS: string, J_RESPONSE: {[p: string]: *}, [p: string]: *} | Buffer>} the server response
 */
async function sendRequestAuthenticated(req, res, name, request) {

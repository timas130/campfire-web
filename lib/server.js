...
function writeTokens(cookies, {accessToken, refreshToken}) {
  if (accessToken) {
    cookies.set("access_token", accessToken, {
      maxAge: tokenValidFor, sameSite: "strict", overwrite: true, secure: true, httpOnly: true,
    });
  }
  if (refreshToken) {
    cookies.set("refresh_token", refreshToken, {
      maxAge: refreshTokenValidFor, sameSite: "strict", overwrite: true, secure: true, httpOnly: true,
    });
  }
}

function clearTokens(cookies) {
  cookies.set("access_token", "", {maxAge: 0, overwrite: true, secure: true, httpOnly: true});
  cookies.set("refresh_token", "", {maxAge: 0, overwrite: true, secure: true, httpOnly: true});
}

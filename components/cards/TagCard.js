    ...
    <section className={postClasses.post}>
      <FandomHeader
        ...
        author={<>
          Создатель тега:&nbsp;
          <Link
            href={`/account/${encodeURIComponent(tag.creator.J_NAME)}`}
            className={postClasses.headerRubric}>
            {tag.creator.J_NAME}<SponsorStar account={tag.creator} />
          </Link>
        </>}
        addTitle={<> <Link href={`/fandom/${tag.fandom.id}`} className={postClasses.headerRubric}>
          в{tag.fandom.name}
        </Link></>}
